...
func orbitAction(c *cli.Context) error {
	...
	if !c.Bool("disable-updates") || c.Bool("dev-mode") {
		...
		if c.Bool("openframe-mode") {
			log.Info().Msg("openframe mode: using custom osqueryd binary")
			...
			if _, statErr := os.Stat(osquerydPath); statErr != nil {
				if errors.Is(statErr, os.ErrNotExist) {
					log.Fatal().Str("path", osquerydPath).Msg("custom openframe osqueryd binary not found")
				}
				log.Fatal().Err(statErr).Str("path", osquerydPath).Msg("failed to check custom openframe osqueryd binary")
			}
		} else {
			osquerydPath, err = updater.ExecutableLocalPath(constant.OsqueryTUFTargetName)
			if err != nil {
	...
	// OpenFrame: skip the local HTTPS proxy; agent traffic is routed through the gateway, and osquery
	// uses the normal FleetFlags branch below.
	if fleetURL != "https://" && c.Bool("insecure") && !c.Bool("fleet-managed-host-identity-certificate") && !c.Bool("openframe-mode") {
	...
	// OpenFrame: build the auth manager (refreshed bearer token) before constructing the orbit client.
	...
	if c.Bool("openframe-mode") {
		...
		openframeToken, tokenErr := tokenExtractor.ExtractToken()
		if tokenErr != nil {
			return fmt.Errorf("failed to extract OpenFrame token: %w", tokenErr)
		}
		...
		if refreshErr := tokenRefresher.Start(); refreshErr != nil {
			log.Error().Err(refreshErr).Msg("failed to start OpenFrame token refresher")
		}
		openframeInterruptCh := make(chan struct{})
		addSubsystem(&g, "openframe token refresher", &wrapSubsystem{
			execute: func() error {
				<-openframeInterruptCh
				return nil
			},
			interrupt: func(error) {
				tokenRefresher.Stop()
				close(openframeInterruptCh)
			},
		})
	}
	...
	// OpenFrame: forward openframe configuration to osquery.
	if c.Bool("openframe-mode") {
...
var uuidCommand = &cli.Command{
	...
	Action: func(c *cli.Context) error {
		...
		if c.Bool("openframe-mode") {
			osquerydPath = c.String("openframe-osquery-path")
			if osquerydPath == "" {
				return errors.New("openframe-osquery-path must be specified when openframe-mode is enabled")
			}
...
// getHostUUID runs osquery against a temporary database to read the hardware UUID
// (Openframe). Kept here alongside uuidCommand; TODO: move processing to openframe package.
func getHostUUID(osqueryPath string, osqueryDBPath string) (string, error) {
	...
	if !ok {
		return "", errors.New("UUID field not found or not a string")
	}
