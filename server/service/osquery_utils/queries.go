...
var extraDetailQueries = map[string]DetailQuery{
	...
	"certificates_windows": {
		...
		Query: `
	SELECT
		ca, hex(common_name) AS common_name_hex, hex(subject2) AS subject2_hex, hex(issuer2) AS issuer2_hex,
		key_algorithm, key_strength, key_usage, signing_algorithm,
		not_valid_after, not_valid_before,
...
// certificateDNColumnsWindows is the Windows column set: osquery's subject2/issuer2 preserve the
// DN attribute keys, so the Windows detail query hex-encodes those instead of subject/issuer.
var certificateDNColumnsWindows = []string{"common_name", "subject2", "issuer2"}
...
func decodeCertificateDNColumns(ctx context.Context, logger *slog.Logger, row map[string]string, cols []string) {
...
func directIngestHostCertificatesDarwin(
	...
	for _, row := range rows {
		...
		decodeCertificateDNColumns(ctx, logger, row, certificateDNColumns)
		// <<< OPENFRAME(waf-inventory-shape)
...
func directIngestHostCertificatesWindows(
	...
	for _, row := range rows {
		// >>> OPENFRAME(waf-inventory-shape): the DN columns arrive hex-encoded from the detail
		// query and must be decoded before use; the decode also unescapes the \xHH sequences
		// upstream handles inline — openframe/docs/agent-inventory-waf-shape.md
		decodeCertificateDNColumns(ctx, logger, row, certificateDNColumnsWindows)
		// <<< OPENFRAME(waf-inventory-shape)
