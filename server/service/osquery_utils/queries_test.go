...
func TestDirectIngestHostCertificatesHexEncodedDN(t *testing.T) {
	const (
		commonName = "Ловушка"
		// macOS keychain DNs are slash-separated; the Windows subject2/issuer2 columns are
		// comma-separated attribute pairs. Each platform's parser only understands its own form.
		subjectDarwin  = `/C=US/ST=California/O=Acme, Inc./CN=Ловушка`
		issuerDarwin   = `/C=US/O=Acme CA/CN=Acme Root`
		subjectWindows = `CN=Ловушка, O=Acme Inc, ST=California, C=US`
		issuerWindows  = `CN=Acme Root, O=Acme CA, C=US`
	)
	...
	for _, tc := range []struct {
		name           string
		columns        func(subject, issuer string) map[string]string
		...
		for _, platform := range []string{"darwin", "windows"} {
			t.Run(tc.name+"/"+platform, func(t *testing.T) {
				...
				subject, issuer := subjectDarwin, issuerDarwin
				if platform == "windows" {
					subject, issuer = subjectWindows, issuerWindows
				}

				row := baseRow()
				for k, v := range tc.columns(subject, issuer) {
					// Windows reads osquery's subject2/issuer2 columns, so the detail query
					// hex-encodes those instead of subject/issuer.
					if platform == "windows" {
						k = strings.Replace(k, "subject", "subject2", 1)
						k = strings.Replace(k, "issuer", "issuer2", 1)
					}
					row[k] = v
				}
				...
				ds.UpdateHostCertificatesFunc = func(ctx context.Context, hostID uint, hostUUID string,
					...
					observedScopes []fleet.HostCertificateScope,
				) error {
					require.Len(t, certs, 1)
					assert.Equal(t, tc.wantCommonName, certs[0].CommonName)
					assert.Equal(t, tc.wantSubjectCN, certs[0].SubjectCommonName)
					assert.Equal(t, tc.wantIssuerCN, certs[0].IssuerCommonName)
					return nil
				}
