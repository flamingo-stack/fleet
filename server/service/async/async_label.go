...
// Redis-key builders for label-membership async batches. They were
// package-level constants of the form "label_membership:..." and
// "label_membership:{%d}"; the functions below preserve the original
// literal shapes (including the {hostID} cluster hash tag) but route
// through a KeyBuilder so the per-tenant prefix is prepended uniformly.
// The hash tag stays inside the braces, ensuring the host-keyed pair
// (set + reported timestamp) still co-locates on the same Redis Cluster
// slot.
func labelMembershipActiveHostIDsKey(kb redis.KeyBuilder) string {
	return kb.Key("label_membership:active_host_ids")
}

func labelMembershipHostKey(kb redis.KeyBuilder, hostID uint) string {
	return kb.Sprintf("label_membership:{%d}", hostID)
}

func labelMembershipReportedKey(kb redis.KeyBuilder, hostID uint) string {
	return kb.Sprintf("label_membership_reported:{%d}", hostID)
}
...
func (t *Task) RecordLabelQueryExecutions(ctx context.Context, host *fleet.Host, results map[uint]*bool, ts time.Time, deferred bool) error {
	...
	keySet := labelMembershipHostKey(t.kb, host.ID)
	keyTs := labelMembershipReportedKey(t.kb, host.ID)
  ...
  `)
	...
	if _, err := storePurgeActiveHostID(t.pool, labelMembershipActiveHostIDsKey(t.kb), host.ID, ts, ts.Add(-ttl)); err != nil {
...
func (t *Task) collectLabelQueryExecutions(ctx context.Context, ds fleet.Datastore, pool fleet.RedisPool, stats *collectorExecStats) error {
	...
	hosts, err := loadActiveHostIDs(pool, labelMembershipActiveHostIDsKey(t.kb), cfg.RedisScanKeysCount)
	if err != nil {
	...
	getKeyTuples := func(hostID uint) (inserts, deletes [][2]uint, err error) {
		keySet := labelMembershipHostKey(t.kb, hostID)
	...
	if len(hosts) > 0 {
		...
		// results to process.
		if _, err := removeProcessedHostIDs(pool, labelMembershipActiveHostIDsKey(t.kb), hosts); err != nil {
...
func (t *Task) GetHostLabelReportedAt(ctx context.Context, host *fleet.Host) time.Time {
	...
	if cfg.Enabled {
		...
		key := labelMembershipReportedKey(t.kb, host.ID)
