...
// hostIdentAllowedOrderKeys restricts ORDER BY for the OpenFrame host-assignment list endpoints
// (ListPolicyHosts / ListQueryHosts), which both SELECT h.id, h.hostname.
var hostIdentAllowedOrderKeys = common_mysql.OrderKeyAllowlist{
	"id":       "h.id",
	"hostname": "h.hostname",
}
...
func (ds *Datastore) ListPolicyHosts(ctx context.Context, policyID uint, opts fleet.ListOptions) ([]fleet.HostIdent, *fleet.PaginationMetadata, error) {
	...
	stmt, args, err := appendListOptionsWithCursorToSQLSecure(stmt, args, &opts, hostIdentAllowedOrderKeys)
	if err != nil {
		return nil, nil, ctxerr.Wrap(ctx, err, "append list options for policy hosts")
	}
...
func (ds *Datastore) policyQueriesForHostInScope(ctx context.Context, host *fleet.Host, restrictToPolicyIDs []uint) (map[string]string, error) {
	...
	if fleet.IsOpenframeMode() {
		// OpenFrame: restrict policies to hosts explicitly assigned via policy_hosts.
		stmt += `
			AND EXISTS (
