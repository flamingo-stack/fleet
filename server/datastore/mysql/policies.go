...
var policyHostsAllowedOrderKeys = common_mysql.OrderKeyAllowlist{
	"id":       "h.id",
	"hostname": "h.hostname",
}
...
func (ds *Datastore) ListPolicyHosts(ctx context.Context, policyID uint, opts fleet.ListOptions) ([]fleet.HostIdent, *fleet.PaginationMetadata, error) {
	...
	stmt, args, err := appendListOptionsWithCursorToSQLSecure(stmt, args, &opts, policyHostsAllowedOrderKeys)
	if err != nil {
		return nil, nil, ctxerr.Wrap(ctx, err, "append list options to list policy hosts query")
	}
...
// openframeManagedExclusion returns the WHERE fragment that keeps OpenFrame-managed policies out of
// a listing. Unconditional: `prepare db` always runs MigrateOpenframe, so the column exists in every
// deployment — FLEET_OPENFRAME_MODE gates behavior, not schema.
func openframeManagedExclusion(alias string) string {
...
func listPoliciesDB(ctx context.Context, q sqlx.QueryerContext, teamID *uint, opts fleet.ListOptions, filterClause string, filterArgs []any) ([]*fleet.Policy, error) {
	...
	// >>> OPENFRAME(managed-policies): openframe/docs/managed-policies.md
	query += openframeManagedExclusion("p")
	// <<< OPENFRAME(managed-policies)
