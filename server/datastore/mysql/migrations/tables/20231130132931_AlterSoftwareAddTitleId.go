...
func Up_20231130132931(tx *sql.Tx) error {
	...
	if !columnExists(tx, "software", "title_id") {
		if _, err := tx.Exec(`ALTER TABLE software ADD COLUMN title_id INT(10) UNSIGNED DEFAULT NULL`); err != nil {
			return fmt.Errorf("failed to add title_id column to software table: %w", err)
		}
	}

	if !indexExistsTx(tx, "software", "title_id") {
		if _, err := tx.Exec(`ALTER TABLE software ADD INDEX (title_id)`); err != nil {
