...
func Up_20260106000000(tx *sql.Tx) error {
	...
	if !columnExists(tx, "host_certificate_templates", "not_valid_before") {
		if _, err := tx.Exec(`
	...
	// Using DATETIME (not DATETIME(6)) since X.509 cert validity has max 1-second precision per RFC 5280
	if !columnExists(tx, "host_certificate_templates", "not_valid_after") {
		if _, err := tx.Exec(`
		ALTER TABLE host_certificate_templates
	...
	if !columnExists(tx, "host_certificate_templates", "serial") {
		if _, err := tx.Exec(`
		ALTER TABLE host_certificate_templates
	...
	if !indexExistsTx(tx, "host_certificate_templates", "idx_host_certificate_templates_not_valid_after") {
		if _, err := tx.Exec(`
		ALTER TABLE host_certificate_templates
