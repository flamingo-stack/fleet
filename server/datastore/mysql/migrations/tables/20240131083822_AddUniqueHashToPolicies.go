...
func Up_20240131083822(tx *sql.Tx) error {
	...
	if !columnExists(tx, "policies", "checksum") {
		...
		if _, err := tx.Exec(`ALTER TABLE policies ADD COLUMN checksum BINARY(16) DEFAULT NULL`); err != nil {
	...
	`,
		...
		if !indexExistsTx(tx, "policies", "idx_policies_checksum") {
			if _, err := tx.Exec(`ALTER TABLE policies ADD UNIQUE INDEX idx_policies_checksum (checksum)`); err != nil {
				return fmt.Errorf("failed to add idx_policies_checksum to policies table: %w", err)
			}
		}
	}
	...
	if indexExistsTx(tx, "policies", "idx_policies_unique_name") {
		if _, err := tx.Exec(`ALTER TABLE policies DROP INDEX idx_policies_unique_name`); err != nil {
