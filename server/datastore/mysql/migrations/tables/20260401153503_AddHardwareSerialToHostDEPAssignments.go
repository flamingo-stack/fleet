...
func Up_20260401153503(tx *sql.Tx) error {
	...
	if !columnExists(tx, "host_dep_assignments", "hardware_serial") {
		_, err := tx.Exec(`
	...
	if !indexExistsTx(tx, "host_dep_assignments", "idx_hdep_hardware_serial") {
		_, err := tx.Exec(`
		ALTER TABLE host_dep_assignments
			ADD INDEX idx_hdep_hardware_serial (hardware_serial)
	`)
		if err != nil {
			return errors.Wrap(err, "add host_dep_assignments.hardware_serial index")
		}
	}
