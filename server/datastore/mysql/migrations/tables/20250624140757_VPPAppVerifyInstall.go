...
func Up_20250624140757(tx *sql.Tx) error {
	...
	if !columnsExists(tx, "host_vpp_software_installs", "verification_command_uuid", "verification_at", "verification_failed_at") {
		if _, err := tx.Exec(`
	...
	} else if !indexExistsTx(tx, "host_vpp_software_installs", "idx_host_vpp_software_installs_verification") {
		if _, err := tx.Exec(`
	ALTER TABLE host_vpp_software_installs
		ADD INDEX idx_host_vpp_software_installs_verification ((verification_at IS NULL AND verification_failed_at IS NULL))
		`); err != nil {
			return fmt.Errorf("failed to add idx_host_vpp_software_installs_verification: %w", err)
		}
	}
...
SET
	...
	`, fleet.MDMAppleStatusError, fleet.MDMAppleStatusCommandFormatError); err != nil {
		return fmt.Errorf("failed to set existing vpp install verification statuses: %w", err)
	}

	if !indexExistsTx(tx, "jobs", "idx_jobs_name_state") {
		if _, err := tx.Exec(`
ALTER TABLE jobs
