...
func Up_20211116184030(tx *sql.Tx) error {
	...
	if !columnsExists(tx, "policies", "name", "query", "description", "author_id", "platforms") {
	...
	`); err != nil {
		...
		if !indexExistsTx(tx, "policies", "idx_policies_author_id") {
			if _, err := tx.Exec(`ALTER TABLE policies ADD KEY idx_policies_author_id (author_id)`); err != nil {
				return errors.Wrap(err, "adding idx_policies_author_id")
			}
		}
		if !indexExistsTx(tx, "policies", "idx_policies_team_id") {
			if _, err := tx.Exec(`ALTER TABLE policies ADD KEY idx_policies_team_id (team_id)`); err != nil {
				return errors.Wrap(err, "adding idx_policies_team_id")
			}
		}
	}

	if !constraintExists(tx, "policies", "policies_queries_ibfk_1") {
		if _, err := tx.Exec(`ALTER TABLE policies
		ADD CONSTRAINT policies_queries_ibfk_1 FOREIGN KEY (author_id) REFERENCES users (id) ON DELETE SET NULL
	`); err != nil {
			return errors.Wrap(err, "adding FK policies_queries_ibfk_1 to 'policies'")
		}
	}
	...
	if !indexExistsTx(tx, "policies", "idx_policies_unique_name") {
		if _, err := tx.Exec(
	...
	if columnExists(tx, "policies", "query_id") {
