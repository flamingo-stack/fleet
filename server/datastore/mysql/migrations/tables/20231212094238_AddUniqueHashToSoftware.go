...
func Up_20231212094238(tx *sql.Tx) error {
	...
	if !columnExists(tx, "software", "checksum") {
		...
		// checksum is important, we will need to use the same everywhere. The canonical
		// logic lives in fleet.Software.ComputeRawChecksum, but we intentionally inline
		// the SQL here rather than reuse it: if that function's implementation changes in
		// the future, it should not retroactively change this historical migration (e.g.
		// it might reference columns that don't exist at the point in time this runs).
		if _, err := tx.Exec(`
UPDATE
	software
SET
