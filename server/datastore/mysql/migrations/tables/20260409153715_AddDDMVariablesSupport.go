...
func Up_20260409153715(tx *sql.Tx) error {
	...
	if !columnExists(tx, "host_mdm_apple_declarations", "variables_updated_at") {
		if _, err := tx.Exec(`ALTER TABLE host_mdm_apple_declarations ADD COLUMN variables_updated_at DATETIME(6) DEFAULT NULL`); err != nil {
	...
	if !constraintExists(tx, "mdm_configuration_profile_variables", "idx_mdm_config_profile_vars_apple_decl_variable") {
		if _, err := tx.Exec(`
		ALTER TABLE mdm_configuration_profile_variables
	...
	if !constraintExists(tx, "mdm_configuration_profile_variables", "fk_mdm_configuration_profile_variables_apple_declaration_uuid") {
		if _, err := tx.Exec(`
		ALTER TABLE mdm_configuration_profile_variables
	...
	if constraintExists(tx, "mdm_configuration_profile_variables", "ck_mdm_configuration_profile_variables_apple_or_windows") {
		if _, err := tx.Exec(`
		ALTER TABLE mdm_configuration_profile_variables
	...
	if !constraintExists(tx, "mdm_configuration_profile_variables", "ck_mdm_configuration_profile_variables_exactly_one") {
		if _, err := tx.Exec(`
		ALTER TABLE mdm_configuration_profile_variables
