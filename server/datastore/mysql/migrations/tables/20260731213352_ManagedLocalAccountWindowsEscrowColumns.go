...
func Up_20260731213352(tx *sql.Tx) error {
	// Idempotent migration. The MODIFYs are naturally idempotent, so the added column gates both.
	if !columnExists(tx, "host_managed_local_account_passwords", "client_error") {
		if _, err := tx.Exec(
	...
	if !columnExists(tx, "mdm_windows_enrollments", "managed_local_account_escrowed") {
		if _, err := tx.Exec(
