...
func Up_20260316120005(tx *sql.Tx) error {
	...
	if indexExistsTx(tx, "software_titles_host_counts", "idx_software_titles_host_counts_team_counts_title") {
		_, err := tx.Exec(`
	...
	if !indexExistsTx(tx, "software_titles_host_counts", "idx_software_titles_host_counts_team_global_hosts") {
		_, err := tx.Exec(`
