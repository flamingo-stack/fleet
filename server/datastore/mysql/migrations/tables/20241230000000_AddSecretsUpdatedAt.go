...
func Up_20241230000000(tx *sql.Tx) error {
	...
	if !columnExists(tx, "mdm_apple_configuration_profiles", "secrets_updated_at") {
	...
	if !columnExists(tx, "host_mdm_apple_profiles", "secrets_updated_at") {
	...
	// secrets_updated_at are updated when profile contents have not changed but secret variables in the profile have changed
	if !columnExists(tx, "mdm_apple_declarations", "secrets_updated_at") {
	...
	if !columnExists(tx, "host_mdm_apple_declarations", "secrets_updated_at") {
