...
type PolicyData struct {
	...
	// HostsIncludeAny restricts this policy to run only on the specified hosts (openframe mode). Read-only.
...
// HostIdent identifies a host by ID and hostname. Used by OpenFrame host-targeting
// (FLEET_OPENFRAME_MODE) for the policy_hosts / query_hosts read paths.
