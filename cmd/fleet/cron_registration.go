...
func registerCleanupAndMaintenanceCrons(ctx context.Context, deps cronSchedulesDeps) {
	...
	// >>> OPENFRAME(query-results-ttl): time-based query_results cleanup (openframe mode only) — openframe/docs/query-results-ttl-cleanup.md
	if fleet.IsOpenframeMode() && deps.config.Server.QueryResultsTTL > 0 {
		deps.register("failed to register query_results_ttl_cleanup schedule", func() (fleet.CronSchedule, error) {
			return newQueryResultsTTLCleanupSchedule(ctx, deps.instanceID, deps.ds, deps.config, deps.logger)
		})
	}
