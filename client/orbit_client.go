...
import (
	...
	"github.com/fleetdm/fleet/v4/pkg/openframe"
...
type OrbitClient struct {
	*BaseClient
	...
	// OpenFrame mode (FLEET_OPENFRAME_MODE on the agent): routes traffic through the
	// gateway tools-agent prefix and attaches a refreshed bearer token to requests.
	openFrameMode bool
...
func (oc *OrbitClient) requestWithExternal(verb string, pathOrURL string, params any, resp any, external bool) error {
	...
	if external {
		...
		if oc.openFrameMode && oc.authManager != nil {
			if authToken := oc.authManager.GetToken(); authToken != "" {
				request.Header.Add("Authorization", "Bearer "+authToken)
			} else {
				log.Debug().Msg("openframe authToken is empty, not adding Authorization header")
			}
		}
	}
...
func NewOrbitClient(
	...
	urlPrefix := ""
	if openFrameMode {
		log.Info().Msg("openframe mode: routing agent traffic through the gateway tools-agent prefix")
